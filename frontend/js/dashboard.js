...
function setupPeriodControls() {
  ...
  $("#startDate").addEventListener("change", async () => { start = new Date($("#startDate").value); await refreshAll(); });
  $("#endDate").addEventListener("change", async () => { end = new Date($("#endDate").value); await refreshAll(); });
}
...
function setupForms() {
  ...
  contribForm.addEventListener("submit", async (e) => {
    ...
    try {
      await api("/goals/contribute", "POST", payload);
      contribModal.hide();
      contribForm.reset();
...
function toastMsg(msg){ $("#toastMsg").textContent = msg; toast.show(); }
...
async function loadTransactionsAndCharts(){
  const res = await api(`/transactions/all?start_date=${fmt(start)}&end_date=${fmt(end)}&page_size=2000`);
  ...
  // build day map for selected period
  const dayMap = {};
  ...
  // MTD category doughnut (use month of 'end')
  const monthStart = new Date(end.getFullYear(), end.getMonth(), 1);
  const mtd = txns.filter(t => new Date(t.created_at) >= monthStart && t.type === "expense");
  const catMap = {};
  mtd.forEach(t => { const k = t.category || "Uncategorized"; catMap[k]=(catMap[k]||0)+Number(t.amount); });
...
function renderLine(labels, inc, exp){
  ...
  lineChart = new Chart($("#incomeExpenseChart").getContext("2d"), {
    ...
    data: { labels, datasets: [
      { label: "Income", data: inc, fill: true, tension: .3 },
      { label: "Expenses", data: exp, fill: true, tension: .3 }
    ]},
...
async function loadBudgetProgress(){
  ...
  const box = $("#budgetProgress"); box.innerHTML = "";
...
async function loadAlerts(){
  ...
  const menu = $("#notifMenu"); const badge = $("#notifBadge");
  ...
  if (alerts.length === 0) {
    ...
    alerts.forEach(a=>{
      const item = document.createElement("div"); item.className = "dropdown-item text-wrap";
      item.textContent = a.message; menu.appendChild(item);
    });
...
async function loadInsights(){
  ...
  const box = $("#insights-container"); box.innerHTML = "";
  (r.advice || []).forEach(a=>{
...
async function loadRecentTx(){
  // last 10 by date within selected range
  const r = await api(`/transactions/all?start_date=${fmt(start)}&end_date=${fmt(end)}&page_size=10`);
  const tbody = $("#recentTx"); tbody.innerHTML = "";
...
async function loadGoals(){
  ...
  const box = $("#goalsList"); box.innerHTML = "";
...
// --- Browser notifications for current alerts ---
async function notifyNow(sendEmail=false){
  try{
    const res = await api("/notify/check", "GET");
    const items = [...(res.alerts||[]), ...(res.goals||[])];
    if (!items.length) return;
    // show native notifications if permitted
    ...
    if (sendEmail){
      await api("/notify/dispatch", "POST"); // email summary
    }
  }catch(e){ console.error(e); }
}
// Example: call on load and allow a bell button to trigger email dispatch
notifyNow(false);
